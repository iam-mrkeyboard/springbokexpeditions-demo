...
import { sendPlanTripEmail } from '@/lib/email';
...
export const POST: APIRoute = async ({ request, clientAddress }) => {
  try {
    ...
    if (!tripType || !firstName || !lastName || !email) {
      return new Response(
        JSON.stringify({
          error: 'Please fill in all required fields.',
          field: !tripType ? 'tripType' : !firstName ? 'firstName' : !lastName ? 'lastName' : !email ? 'email' : 'general',
        }),
        {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
    ...
    const emailRes = await sendPlanTripEmail({
      tripType,
      ...
      kids: kids ? parseInt(String(kids)) : undefined,
    ...
    if (!emailRes.success) {
      return new Response(
        JSON.stringify({
          error: 'Failed to send trip plan. Please try again or WhatsApp us directly.',
        }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json' },
    ...
    console.error('[API /api/plan-trip] Internal error:', err);
